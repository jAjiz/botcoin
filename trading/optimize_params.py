...
def _parse_args() -> dict:
    ...
    if not args["pair"] or not args["mode"]:
        print(
            ...
            "[FEE_PCT=0.00] [START=YYYY-MM-DD] [END=YYYY-MM-DD] [MIN_OPS=0] [MIN_TEST_OPS=0] "
            "[TRAIN_SPLIT=1.00] [SPLIT_METHOD=RESET|CONTINUE|BOTH] "
        )
        sys.exit(1)
...
def main() -> None:
    ...
    if mode == "CURRENT":
        ...
        if test_df.empty:
            print("\n=== SCORE (in-sample) ===")
            print(f"Train: pnl={in_sample_score.total_pnl:.2f}% | ops={in_sample_score.ops}")
            return
        ...
        if split_method == "RESET":
            ...
            print(f"CONTINUE Test : pnl={test_cont.total_pnl:.2f}% | ops={test_cont.ops}")
    ...
    print(f"Testing {len(candidates)} combinations (exhaustive)...")
    ...
    print(f"Passed train ops: {passed_train} | Passed test ops: {passed_test}")
    ...
    if split_method == "BOTH":
        ...
        print(f"CONT Train: pnl={cont_train_best.total_pnl:.2f}% | ops={cont_train_best.ops}")
    ...
    for i, (k, ins, tr, te, c) in enumerate(evaluated[:5], start=1):
        ...
        if split_method != "BOTH":
            ...
            print(
                f"{i:>2}. robust={r[0]:.1f}% | "
                f"ins={ins.total_pnl:.1f}% | train={tr.total_pnl:.1f}% | "
                f"test={te.total_pnl:.1f}% | {act}"
            )
            continue
        ...
        print(
            f"{i:>2}. overall={overall_top[0]:.1f}% | "
            f"reset={robust_reset_top[0]:.1f}% | cont={robust_cont_top[0]:.1f}% | "
            f"ins={ins_top.total_pnl:.1f}% | {act}"
        )
    print("\nSuggested .env lines (use single-underscore names):")
    for line in _format_env_lines(pair, best_cand):
        print(line)
