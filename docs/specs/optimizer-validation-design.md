...
### The forced rebuy: where the loss lands, not what causes it (2026-09-10)

**This section replaces an earlier one whose figures were wrong.** The first pass identified
forced rebuys with `op.idx in mask`, but `Operation.idx` is `len(ops) + 1` — the operation's
*ordinal*, not its bar index. That compares an ordinal against a set of bar indices, so it
classified by coincidence: a gate open 6.7 % of the time has its early bars masked and every
cycle came out "forced", while one open 77.7 % came out with none. The retracted table claimed
zero forced rebuys under the symmetric gate; there are 22 of 129, 42 of 154 and 40 of 174. Bar
identity now comes from the operation's timestamp.

**Forced cycles are where the loss sits.** Splitting each cycle by whether its closing buy
landed on a masked bar, and compounding each group in base asset:

| year | gate | config | cycles | forced | compounded, forced | compounded, free |
|---|---|---|---|---|---|---|
| 2023 | symmetric | `mm=0.020 s=0.9` | 9 | 4 | **−8.3 %** | **+10.4 %** |
| 2023 | wide up-only | `mm=0.000 s=0.9` | 28 | 9 | **−25.2 %** | **+17.7 %** |
| 2024 | symmetric | `mm=0.010 s=0.9` | 20 | 15 | **−26.8 %** | **+8.4 %** |
| 2025 | wide up-only | `mm=0.020 s=0.9` | 6 | 2 | **−12.8 %** | **+13.6 %** |
| 2025 | symmetric | `mm=0.020 s=0.9` | 11 | 7 | **−18.9 %** | **+5.5 %** |

The pattern holds across gates, years and configs: **the cycles the mask closes lose, and the
cycles the trailing stop closes on its own are frequently positive.** The gate shuts on an
impulse, so a bot caught in cash is bought back into a move that has already happened.

**And removing the forcing does not help.** `trading/engine.py` gained a fourth inert switch,
`force_hold_rebuy` (default `True`, production-neutral): when off, the cash leg is left alone
and resumes when the mask lifts. One switch between arms, everything else identical, 36 pairs
across three gates, three years and four configs:

| | forcing better | not forcing better |
|---|---|---|
| 2023 | 10 | 1 |
| 2024 | 5 | 7 |
| 2025 | 1 | 11 |
...
A coin flip, decided by the year. Nothing crosses holding in either arm — the best figures are
+9.4 % and +4.8 % without forcing and +0.8 % with it, all inside the noise this document has
been measuring all along.

**So the hypothesis was right as a description and wrong as a remedy.** The forced rebuy is
where the loss lands, not what creates it: take it away and the loss reappears somewhere else.
That is exactly what the martingale argument predicts — every cycle is a zero-expectation draw
minus fees, and which particular cycles carry the loss is bookkeeping. The first-order
...
### Choosing the gate for structure instead of flatness (2026-09-10)

The gate was selected to admit the *flattest* market it could, and the admitted market came out
a random walk. The owner asked the obvious next thing: was the objective wrong — is there a gate
that lets more variance through, enough that something can be harvested from it? Measured across
all 154 causal variants and eight years, with the shuffled control this document now requires.

**There is real structure, and it is concentrated in one family.** VR inside the admitted
stretches minus the VR of the *same variant applied to the same path with its returns shuffled*
— the only part the selection artefact cannot explain:

| variant | coverage | worst admitted drift | 4 h | 12 h | 1 d | 2 d |
|---|---|---|---|---|---|---|
| `er n=10 t=0.2 d=0` | 33.2 % | **383 %** | −0.197 | −0.211 | −0.249 | −0.210 |
| `er n=20 t=0.2 d=0` | 44.0 % | 115 % | −0.103 | −0.190 | −0.191 | −0.217 |
| `bajo max n=30 p=0.05 d=0` | 55.6 % | 92 % | −0.116 | −0.141 | −0.101 | −0.146 |
| `er n=20 t=0.3 d=0` | 61.0 % | **30 %** | −0.097 | −0.146 | −0.193 | −0.263 |
| `impulso m=0.07 k=7` (incumbent) | 53.5 % | 50 % | ≈0 | ≈0 | −0.04 to −0.14 | ≈0 |

So the answer to the question as asked is **yes**: gates exist that admit two to six times more
genuine mean reversion than the one selected for flatness. The efficiency-ratio family is where
it lives, which makes sense — `er` selects directly on how much of the path the net move
consumes, and that is close to selecting on VR itself.

**And it is not enough to pay for itself, by a wide margin.** The money version of the same
screen — best of the two naive bets at horizon h (contrarian or continuation, whichever is
positive), on non-overlapping returns inside stretches, minus the 0.80 % round trip, ranked by
worst year over the 108 variants with ≥ 15 % coverage in all eight years:

| variant | coverage | worst drift | best h | **worst year, net** | years > 0 |
|---|---|---|---|---|---|
| `impulso m=0.07 k=5` | 64.8 % | 100 % | 2 d | **−0.55 %** | 1/7 |
| `alcista m=0.05 k=7` | 61.2 % | 100 % | 2 d | −0.58 % | 2/8 |
| `er n=30 t=0.3 d=0` | 66.5 % | 37 % | 2 d | −0.67 % | 2/8 |
| `impulso m=0.07 k=7` (incumbent) | 53.5 % | 50 % | 2 d | −0.71 % | 1/7 |

**0 of 108 have a positive worst year.** The best variant in the catalogue, at its best horizon,
in its worst year, falls 55 basis points short of the fee. And the structure and the drift move
together: the variants with the most reversion admit 383 %, 229 % and 191 % annualised drift,
which is the trade being proposed — more variance in exchange for more trend — priced out
explicitly.

**Taken end to end, it changes nothing.** The two gates with the most structure were added to
the sweep and given the full 105 configurations at 1-min fidelity on 2023-2025:

| gate | selected for | worst drift | positive in all 3 years | worst-year surface |
|---|---|---|---|---|
| `er n=20 t=0.3 d=0` | structure at tolerable drift | 30 % | **0/105** | monotone → −0.4 % at `mm≥0.20` |
| `er n=10 t=0.2 d=0` | maximum structure | 383 % | **0/105** | monotone → −0.4 % at `mm≥0.05` |

Identical shape to the three gates chosen by filter quality: every worst-year surface monotone
in `min_margin`, optimum at the configuration that does not trade. Five gates now, selected by
two *opposite* objectives, all 0/105.

**What this closes.** "Pick a gate that admits more variance" as a route to profitability. The
resource a trailing stop needs is not variance — a driftless random walk has all the variance
you like and zero expectation for any causal stopping rule. What it would need is *predictable*
structure worth more than 80 basis points a round trip, and the largest such structure in the
catalogue is worth about 25 basis points at its best horizon in its best years. The gap is not a
tuning distance.

### It is not the fees, and a signed bet is not a long-only bot (2026-09-10)

The horizon screen left something that looked like a lead: the naive contrarian's gross
expectation grows roughly with the square root of the horizon (+0.011 % at 1 h, +0.183 % at
1 d, +0.742 % at 5 d) while the fee is *fixed per operation*, so the ratio improves by waiting
longer. At five days the gross is 0.742 % against a 0.80 % round trip — a 6-basis-point gap.
The owner asked the right question of it: is the fee the only thing left?

**No, and the answer is not close.** The same rule, run as something the bot could actually
execute — after a block that rises go to cash, after a block that falls buy, long or flat only —
scored in base asset against holding, eight years, medians over eight starting phases:

| horizon | worst year @ 0.40 % | worst year @ 0.25 % | **worst year @ 0 %** | years > 0 @ 0 % |
|---|---|---|---|---|
| 2 d | −72.7 % | −68.7 % | **−60.6 %** | 3/8 |
| 5 d | −66.4 % | −64.7 % | **−61.8 %** | 3/8 |
| 14 d | −70.1 % | −69.7 % | **−69.1 %** | 3/8 |
| 30 d | −59.0 % | −58.8 % | **−58.5 %** | 3/8 |

**At a fee of exactly zero the rule still loses 54-69 % of the base asset in its worst year.**
Removing the entire fee bill is worth 5-15 points at the short horizons and 1-3 points at the
long ones, and it never changes the sign of a single worst year. Going from taker (0.40 %) to
maker (0.25 %) — the largest fee improvement actually available — buys 2-6 points. The gap to
close is sixty.

**Why the gross expectation lied.** `E[-sign(r_t) · r_{t+1}]` is a *signed* bet: it pays equally
for being right about a rise and about a fall. The bot cannot short. Converted to what it can
do, the "wrong" half becomes *sitting in cash*, and in base-asset terms sitting in cash through
a rise is the dominant way to lose. A positive per-operation expectation on a signed bet is
therefore not evidence of anything a long-only bot can reach, and this document should not have
reported one without the conversion. The conversion costs nothing and it reverses the reading.

**And the years it wins are the years it always wins.** Positive in 2018, 2022 and 2025 at every
horizon and every fee level; negative in 2019, 2020, 2021, 2023 and 2024. Those are exactly the
three calendar years in which XBTEUR fell. It is the same conditional instrument the eight-year
`mm=0` run already produced, arrived at from a completely different direction — which is
evidence about the asset, not about the rule.

**On sharpening the gate to lower VR.** Trivially possible and worthless. A tighter band lowers
measured VR *by construction* — that is what the horizon column and the shuffled control both
show — so every sharpening improves the artefact and nothing else. The quantity that could
matter is the excess over a matched control, which `gate_structure_screen.py` maximised across
154 variants; its best is 0.25 VR units and it does not pay.

### Inverting the question: what the bot needs, and whether a gate can deliver it (2026-09-10)

Five gates chosen five ways all returned 0/105, so the owner proposed inverting the design:
measure what the admitted market has to look like for the bot to win, then look for a gate that
delivers it. Run over all 154 catalogue variants x 8 years x 4 configs at 15-min fidelity —
3988 points of "properties of the admitted market → the bot's result in base asset".

**Step 1 says the bot needs the admitted market to fall, and says it monotonically.**

| admitted drift | points | median | p90 | % > 0 | best |
|---|---|---|---|---|---|
| < −60 % | 2008 | −12.0 % | +19.8 % | **29 %** | +130.2 % |
| −60 to −30 % | 468 | −17.7 % | +6.8 % | 18 % | +55.8 % |
| −30 to −10 % | 396 | −17.0 % | +8.9 % | 22 % | +43.2 % |
| −10 to +10 % | 244 | −22.4 % | +1.5 % | 12 % | +32.2 % |
| +10 to +30 % | 208 | −24.3 % | +1.8 % | 12 % | +33.8 % |
| +30 to +60 % | 228 | −34.4 % | −2.6 % | 8 % | +19.8 % |
| +60 to +150 % | 328 | −41.0 % | −6.0 % | 4 % | +50.8 % |
| > +150 % | 108 | −30.9 % | −9.0 % | **0 %** | −3.4 % |

Eight buckets, monotone in both the median and the share positive. And **more variance is worse,
not better** — inside the flat-drift band the share positive falls from 21 % at 30-45 %
annualised volatility to 9 % above 80 %, with the median going from −16.7 % to −51.6 %. That is
the "widen the gate so something exploitable gets in" hypothesis measured directly on the bot
rather than through a variance ratio, and it points the other way.

**Step 2 is the surprise: that requirement is deliverable, causally, eight years out of eight.**
The winning region (10th-90th percentile of the positive points) is admitted drift between
−99 % and −10 %, and fifteen variants land inside it in every one of 2018-2025:

| variant | years in region | admitted drift, 2018 → 2025 |
|---|---|---|
| `alcista m=0.09 k=7` | 8/8 | −98 −84 −84 −98 −87 −12 −54 −41 |
| `bajo max n=20 p=0.05 d=0` | 8/8 | −97 −95 −97 −96 −93 −87 −91 −80 |
| `bajo max n=30 p=0.10 d=0` | 8/8 | −94 −96 −96 −98 −94 −90 −96 −95 |
| `bajo max n=60 p=0.05 d=0` | 8/8 | −78 −78 −61 −92 −72 −62 −70 −51 |

**This was expected to fail and did not.** The prediction going in was that the requirement
would turn out to be forward drift, which no causal rule can supply. It is not: "the market
while the price sits below its recent high" has negative drift *by construction*, so an up-only
gate delivers it without forecasting anything. Selecting on a causal price condition and getting
a stable directional property out of it is not the prediction problem.

**And it still does not produce a profit, which is now the sharper question.** Two of those
exact gates — `bajo max n=20 p=0.10` and `sin max n=10` — already went through the full 105-config
sweep at 1-min fidelity and returned 0/105 on all three years. Even in the best drift bucket the
median is −12.0 % and only 29 % of points clear holding. So negative admitted drift is
**necessary and not sufficient**, and the reason is the same conditioning that makes it
deliverable: the drift is negative *within* the admitted stretches because the stretches end
...
- **The fees are not the binding constraint, and this is now measured at zero.** The slow contrarian, run as a long-or-flat bot and scored in base asset, loses 54-69 % of the base asset in its worst year **with the fee set to zero**; the whole fee bill is worth 5-15 points at 2 days and 1-3 points at 30. Taker-to-maker (0.40 % → 0.25 %) buys 2-6 points against a sixty-point gap. Any future "it would work with lower costs" claim goes through this measurement first.
